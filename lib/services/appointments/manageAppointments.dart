...
class AppointmentsService{
  ...
  Future<void>? bookAppointment( phonenumber, patientid,
      firstname, lastname,
      dateofbirth, email,
      gender, nationalid,
    ...
    final url = Uri.parse('http://localhost:3001/mongo/onlinebooking/');
...
Future<int?> rescheduleAppointment(date, time, department, procedure, service, appointmentid) async{
  try{
    var url = Uri.parse('http://localhost:3001/mongo/onlinebooking/reschedulebooking');
...
Future<int?> cancelAppointment(appointmentid) async{
  try{
    var url = Uri.parse('http://localhost:3001/mongo/onlinebooking/cancelbooking');
